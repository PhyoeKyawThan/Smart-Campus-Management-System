...
async function fetch_guests(limit, offset, datas) {
    ...
    guests.forEach(guest => {
        ...
        <tr>
          ...
          <td class="text-center font-bold flex flex-row justify-center"><img src="/static/profiles/guest/${guest.picture_uri}" alt=""
              class="block w-20 h-20 bg-slate-300"></td>
          <td class="text-center font-bold">${guest.name}</td>
          <td class="text-center font-bold">${guest.people_count}</td>
...
async function addNewGuest(){
    ...
    const form = document.getElementById("guest-add-form");
    let data = {
        name: name.value,
        people_count: count.value
    };
    // first upload image
