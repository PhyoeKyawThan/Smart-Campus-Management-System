...
from . import db
from .assets.validate import student_exists, is_admin_in_session, is_not_valid
...
def register_student():
    ...
    if request.method == "POST":
        ...
        if not is_not_valid(student_data):
            return jsonify({
                "status": 403,
...
@student.route("/update_student/<int:student_id>", methods=["PUT"])
def update_student_info(student_id: int):
    ...
    if request.method == "PUT":
        ...
        try:
            ...
            # print(edit_data["picture_uri"])
            if edit_student:
                edit_student.name = edit_data["name"]
                if is_picture:
                    edit_student.picture_uri = is_picture
